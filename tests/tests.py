...
from memory_repo.query import MemoryRepo
...
class TestQueries(unittest.TestCase):
    ...
    def test_filters_lamda(self):
        ...
        self.assertEqual(
            self.repo.filter(age=lambda x: x >= 20 and x <= 30)
            .exclude(gender="male")
            .count(),
            2,
        )
    ...
    def test_filters(self):
        self.assertEqual(
            self.repo.filter(name="Crawford Wilkins")[0].id, ACCOUNT_MODELS[7].id
        )
        self.assertEqual(
    ...
    def test_filters_combine(self):
        ...
        self.assertEqual(
            self.repo.filter(active=True, age__lte=22, gender="female").count(), 1
        )
        self.assertEqual(
            self.repo.filter(active=True, age=22, gender="female").count(), 1
        )
        self.assertEqual(
            self.repo.filter(active=True, age=22, gender="male").count(), 1
        )
        self.assertEqual(
            self.repo.filter(
        ...
        self.assertEqual(
            self.repo.filter(active=True, id=5).first().id, ACCOUNT_MODELS[5].id
        )

    def test_filters_combine_chained(self):
        self.assertEqual(
            self.repo.filter(active=True)
            .filter(age__lte=22)
    ...
    def test_exclude(self):
        self.assertEqual(
            self.repo.exclude(email="crawfordwilkins@vicon.com").count(), 7
        )
    ...
    def test_exclude_combine(self):
        self.assertEqual(
            self.repo.exclude(active=True, age__lte=22, gender="female").count(), 7
        )
        self.assertEqual(
            self.repo.exclude(active=True, age=22, gender="female").count(), 7
        )
    ...
    def test_filter_combined_with_exclude(self):
        self.assertEqual(
            self.repo.filter(active=True).exclude(gender="male").count(), 2
        )
        self.assertEqual(
    ...
    def test_related_lookups(self):
        ...
        self.assertEqual(
            self.repo.filter(friends__gender="male").exclude(active=True).count(), 1
        )

    def test_ordering(self):
        self.assertEqual(list(self.repo.order_by("id")), ACCOUNT_MODELS)
        self.assertEqual(
            list(self.repo.order_by("-id")), list(reversed(ACCOUNT_MODELS))
        )
        self.assertEqual(
