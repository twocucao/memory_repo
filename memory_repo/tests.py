        ...
        for (key, value) in data.items():
            if isinstance(value, (list, tuple)):
                setattr(
                    self,
                    key,
                    [(AccountObject(x) if isinstance(x, dict) else x) for x in value],
                )
            else:
                setattr(
                    self,
                    key,
                    (AccountObject(value) if isinstance(value, dict) else value),
                )
...
class TestQueries(unittest.TestCase):
    ...
    def test_filters_lamda(self):
        self.assertEqual(
            self.queryset.filter(
                company=lambda x: x == "VICON" or x == "ELITA"
            ).count(),
            2,
        )
        self.assertEqual(
            self.queryset.filter(age=lambda x: x >= 20 and x <= 30).count(), 6
        )
        self.assertEqual(
            self.queryset.filter(age=lambda x: x >= 20 and x <= 30)
            .exclude(gender="male")
            .count(),
            2,
        )
    ...
    def test_filters(self):
        self.assertEqual(
            self.queryset.filter(name="Crawford Wilkins")[0].id, ACCOUNT_OBJECTS[7].id
        )
        self.assertEqual(
            self.queryset.filter(address__icontains="London")[0].id,
            ACCOUNT_OBJECTS[0].id,
        )
        self.assertEqual(
            self.queryset.filter(email="crawfordwilkins@vicon.com").count(), 1
        )
    ...
    def test_filters_combine(self):
        self.assertEqual(
            self.queryset.filter(
                active=True, age__lte=22, gender="female", company="FITCORE"
            ).count(),
            1,
        )
        self.assertEqual(
            self.queryset.filter(active=True, age__lte=22, gender="female").count(), 1
        )
        self.assertEqual(
            self.queryset.filter(active=True, age=22, gender="female").count(), 1
        )
        self.assertEqual(
            self.queryset.filter(active=True, age=22, gender="male").count(), 1
        )
        self.assertEqual(
            self.queryset.filter(
                active=True, age=22, gender="male", name="Angeline Holloway"
            ).count(),
            1,
        )
        ...
        self.assertEqual(
            self.queryset.filter(active=True, id=5).first().id, ACCOUNT_OBJECTS[5].id
        )
    ...
    def test_filters_combine_chained(self):
        ...
        self.assertEqual(
            self.queryset.filter(active=True).filter(id=5).first().id,
            ACCOUNT_OBJECTS[5].id,
        )

    def test_exclude(self):
        self.assertEqual(
            self.queryset.exclude(email="crawfordwilkins@vicon.com").count(), 7
        )
    ...
    def test_exclude_combine(self):
        self.assertEqual(
            self.queryset.exclude(active=True, age__lte=22, gender="female").count(), 7
        )
        self.assertEqual(
            self.queryset.exclude(active=True, age=22, gender="female").count(), 7
        )
    ...
    def test_exclude_combine_chained(self):
        self.assertEqual(
            self.queryset.exclude(active=True)
            .exclude(age__lte=22)
            .exclude(gender="male")
            .count(),
            1,
    ...
    def test_filter_combined_with_exclude(self):
        self.assertEqual(
            self.queryset.filter(active=True).exclude(gender="male").count(), 2
        )
        self.assertEqual(
            self.queryset.filter(active=True, gender="male")
            .exclude(age__lte=22)
            .count(),
            2,
        )
    ...
    def test_get(self):
        self.assertEqual(
            self.queryset.get(name="Angeline Holloway"), ACCOUNT_OBJECTS[0]
        )
    ...
    def test_related_lookups(self):
        ...
        self.assertEqual(
            self.queryset.filter(friends__gender="male").exclude(active=False).count(),
            3,
        )
        self.assertEqual(
            self.queryset.filter(friends__gender="male").exclude(active=True).count(), 1
        )

    def test_ordering(self):
        self.assertEqual(list(self.queryset.order_by("id")), ACCOUNT_OBJECTS)
        self.assertEqual(
            list(self.queryset.order_by("-id")), list(reversed(ACCOUNT_OBJECTS))
        )
        self.assertEqual(
            list(self.queryset.order_by("gender")),
            sorted(ACCOUNT_OBJECTS, key=lambda x: x.gender),
        )
        self.assertEqual(
            list(self.queryset.order_by("-gender")),
            sorted(ACCOUNT_OBJECTS, key=lambda x: x.gender, reverse=True),
        )

    def test_orderings(self):
        self.assertEqual(
            list(self.queryset.order_by("friends__gender")),
            sorted(ACCOUNT_OBJECTS, key=lambda x: x.friends.gender),
        )
        self.assertEqual(
            list(self.queryset.order_by("-friends__gender")),
        ...
        self.assertEqual(
            list(self.queryset.order_by("friends__name")),
            sorted(ACCOUNT_OBJECTS, key=lambda x: x.friends.name),
        )
        self.assertEqual(
            list(self.queryset.order_by("-friends__name")),
    ...
    def test_first(self):
        ...
        self.assertEqual(
            self.queryset.filter(email="wardwaller@elita.com").first(),
            self.queryset.filter(email="wardwaller@elita.com")[0],
        )

    def test_range(self):
        queryset = self.queryset.filter(
            registered__range=("2015-09-21", "2016-12-08")
